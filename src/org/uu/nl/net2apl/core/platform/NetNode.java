...
import org.uu.nl.net2apl.core.fipa.ams.ReceiveRemoteAddress;
...
public class NetNode<T extends MessageInterface> implements Messenger<T>, Runnable {
	...
	protected DirectoryFacilitator yellowPages; 
				...
				try(Socket servSocket = serverSocket.accept()) {
					...
					try (DataInputStream dis = new DataInputStream(servSocket.getInputStream())){						
						...
						case -2: // Got a reply with the ID of a DF on another Platform. 
							try {
								AgentID remoteYellowPages = new AgentID(dis.readUTF());
	...
	public void register(Agent agent) {
		if (agent instanceof DirectoryFacilitator) {
	...
	@Override
	public void deliverMessage(AgentID receiver, T message) throws MessageReceiverNotFoundException {
