...
public class ACLMessage implements Cloneable, Serializable, MessageInterface {
	...
	// #MIDP_EXCLUDE_BEGIN
	private Set<AgentID> dests = new HashSet<AgentID>(RECEIVERS_EXPECTED_SIZE);
	private Set<AgentID> reply_to = null;
	// #MIDP_EXCLUDE_END
	/*
	...
	public static ACLMessage decode(byte[] data)
	{
		try {
			...
			Envelope envelope = Envelope.decode(din); //(Envelope) ois.readObject();
	...
	public ACLMessage(int perfIndex) {
		this(Performative.values()[perfIndex]);
	}
	 ...
	 **/
	public ACLMessage(Performative perf) {		
	 ...
	 */
	public void addReceiver(AgentID r) {
		if (r != null) {
			// #MIDP_EXCLUDE_BEGIN
			dests.add(r);
			// #MIDP_EXCLUDE_END
	 ...
	 */
	public boolean removeReceiver(AgentID r) {
		if (r != null) {
			// #MIDP_EXCLUDE_BEGIN
	 ...
	 *            The value to add to the slot value set.
	 */
	public void addReplyTo(AgentID dest) {
		if (dest != null) {
			// #MIDP_EXCLUDE_BEGIN
			reply_to = (reply_to == null ? new HashSet<AgentID>(REPLYTO_EXPECTED_SIZE) : reply_to);
			reply_to.add(dest);
			// #MIDP_EXCLUDE_END
			/*
	 ...
	 * @return true if the AID has been found and removed, false otherwise
	 */
	public boolean removeReplyTo(AgentID dest) {
	 ...
	 */
	public void setContentObject(FIPASendableObject s) throws IOException {
	 ...
	 * @exception UnreadableException
	 *                when an error occurs during the decoding.
	 */
	public FIPASendableObject getContentObject() throws UnreadableException {
	 ...
	 */
	public Iterator<AgentID> getAllReceiver() {
		// #MIDP_EXCLUDE_BEGIN
	 ...
	 *         agents for this message.
	 */
	public Iterator<AgentID> getAllReplyTo() {
		if (reply_to == null) {
	...
	public Performative getPerformative() {
		return Performative.values()[performativeIndex];
	}
	...
	public void setDefaultEnvelope() {
		...
		// #MIDP_EXCLUDE_BEGIN
		Iterator<AgentID> it = dests.iterator();
		// #MIDP_EXCLUDE_END
		/*
	...
	public synchronized Object clone() {
		...
		try {
			...
			if (dests != null) {
				result.dests = new HashSet<AgentID>(dests.size());
				Iterator<AgentID> it = dests.iterator();
				while (it.hasNext()) {
					AgentID id = it.next();
			...
			// Deep clone reply_to
			if (reply_to != null) {
				result.reply_to = new HashSet<AgentID>(reply_to.size());
				Iterator<AgentID> it = reply_to.iterator();
				while (it.hasNext()) {
					AgentID id = it.next();
	...
	public void reset() {
		 ...
		 */
		performativeIndex = Performative.NOT_UNDERSTOOD.index();
		content = null;
		byteSequenceContent = null;
		reply_with = null;
		in_reply_to = null;
	 ...
	 */
	public ACLMessage createReply(AgentID sender) {
		return createReply(sender, getPerformative());
	}
	
	public ACLMessage createForward(Performative perf, AgentID sender, AgentID receiver) {
	...
	public ACLMessage createForward(AgentID sender, AgentID receiver) {
	 ...
	 * @return the ACLMessage to send as a reply
	 */
	public ACLMessage createReply(AgentID sender, Performative perf) {
		ACLMessage m = new ACLMessage(perf);
		m.setSender(sender);
		Iterator<AgentID> it = getAllReplyTo();
		while (it.hasNext())
	...
	public void setReceivers(Collection<AgentID> al) {
		if (dests == null) {
		    dests = new HashSet<>();
		}
		dests.clear();
	...
	// For persistence service
	public void setReplyTo(Collection<AgentID> al) {
		reply_to.clear();
		reply_to.addAll(al);
	}
