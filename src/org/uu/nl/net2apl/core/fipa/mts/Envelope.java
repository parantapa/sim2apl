...
package org.uu.nl.net2apl.core.fipa.mts;
...
import org.uu.nl.net2apl.core.fipa.acl.LEAPACLCodec;
...
public class Envelope implements Serializable {
	...
	private ArrayList<Received> stamps = new ArrayList<Received>(EXPECTED_LIST_SIZE); // received parameter in FIPA
	private ArrayList<Property<?>> properties = new ArrayList<Property<?>>(EXPECTED_LIST_SIZE);
		...
		try {
			...
			if ((flagsA & 0x01) != 0) {
				while(din.readBoolean()) {
					env.to.add( LEAPACLCodec.deserializeAID(din) );
				}
			}
						
			//AgentID from
			if ((flagsA & 0x02) != 0) {
				env.from = LEAPACLCodec.deserializeAID(din);
			}
			...
			if ((flagsA & 0x80) != 0) {
				while (din.readBoolean()) {
					env.intendedReceiver.add( LEAPACLCodec.deserializeAID(din) );
				}
			}
...
//					env.stamps.add( Received.decode(receivedBytes) );
			...
			if ((flagsB & 0x04) != 0) {
				...
				do {
					...
					switch (propType) {
					case 2: prop = new Property<Long>(name, din.readLong()); break;
					case 3: prop = new Property<Double>(name, din.readDouble()); break;
					case 7: prop = new Property<AgentID>(name, LEAPACLCodec.deserializeAID(din)); break;
					
					case 1:
					default: prop = new Property<String>(name, din.readUTF()); break;
					}
	...
	{		
		...
		//AgentID from
		if ((flagsA & 0x02) != 0) {
			LEAPACLCodec.serializeAID(from, dos);
		}
	 ...
	 */
	public void setReceived(Received ro) {
		addStamp(ro);
	}
	 ...
	 */
	public void addStamp(Received ro) {
		stamps.add(ro);
	}
	 ...
	 */
	public Received[] getStamps() {
		Received[] ret = new Received[stamps.size()];
		int counter = 0;

		for (Iterator<Received> it = stamps.iterator(); it.hasNext();)
			ret[counter++] = it.next();
	...
	public Object clone() {
		...
		env.properties = new ArrayList<Property<?>>(properties.size());
